...
for idx = 1:10
    ...
    pts_mov = pts_struct.extreme.e; %b表示0 e表示5
    [volfix, spc] = read_DIR_volume_4dCT(idx, 0, basepth);
    volfix = double(volfix);
    pts_fix = pts_struct.extreme.b; %b表示0 e表示5
    ...
    save(['F:/matlabbb/bin/ct3dreg/DIR_5to0_idx',num2str(idx),'use_refinement',num2str(use_refinement),'_resize',num2str(resize),'_fast_lcc',num2str(fast_lcc),'.mat'], 'voldef', 'Tptv','Kptv','crop_v');
    %     save(['F:/matlabbb/bin/ct3dreg/DIR_5to0_TRE.mat'], 'TRE_e', 'TIME_e');
end
end
end
end